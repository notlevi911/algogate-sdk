...
async function handleMessage(message) {
    ...
    switch (typedMessage?.type) {
        ...
        case "DETECT_PAGE_WITH_BACKEND":
            return {
                ok: true,
                result: await detectPageWithBackend(String(typedMessage.payload?.url ?? ""), String(typedMessage.payload?.html ?? ""))
            };
        case "CONNECT_WALLET":
...
async function deepAnalyzeProtocol(payload) {
    const settings = await getSettings();
...
async function getWalletBalance(address) {
    ...
    const response = await fetch(`https://testnet-api.algonode.cloud/v2/accounts/${encodeURIComponent(address)}`);
...
async function detectPageWithBackend(url, html) {
    if (!url) {
...
function buildMockAnalysis(protocol, pageContext) {
    return [
        "## Overall risk",
