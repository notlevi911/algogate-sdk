...
async function mountUi(state: ContentState, detection: PageActionDetection) {
  ...
  const resizeHandle = panel.querySelector<HTMLElement>("[data-resize-handle]");
...
async function runPageAction(
  ...
  try {
    ...
    setResultMessage(
      state,
      error instanceof Error ? error.message : "This action failed unexpectedly.",
      "error"
    );
  }
}
  ...
  if (!balanceResponse?.ok || (requiresAlgo ? !hasEnoughAlgoForPayment : availableAtomicAmount < requiredAtomicAmount)) {
    setResultMessage(
      ...
      requiresAlgo
        ...
        : `${approvalText}\n\nThis wallet does not have enough USDC to pay yet.`,
      "warning"
    );
    return;
  }
...
function promptForPaymentApproval(
  ...
  return new Promise<boolean>((resolve) => {
    const approveButton = state.resultBox?.querySelector<HTMLButtonElement>("[data-payment-approve]");
    const cancelButton = state.resultBox?.querySelector<HTMLButtonElement>("[data-payment-cancel]");
...
function startPanelResize(state: ContentState, event: MouseEvent) {
  if (!state.panel) {
    return;
  }
...
async function confirmPaymentWithRetry(
  ...
  for (let attempt = 1; attempt <= 20; attempt += 1) {
    setResultMessage(
      state,
      `Payment sent. Waiting for confirmation (${attempt}/20)...`,
      "loading"
    );
